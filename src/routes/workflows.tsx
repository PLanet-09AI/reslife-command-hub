...
import { WORKFLOWS, RESIDENCES, WORKFLOW_STEPS, WORKFLOW_ASSIGNEES } from "@/lib/mock-data";
...
type Step = { name: string; assignee: string; status: WfStatus; updated: string; action: string; attachments?: { name: string }[] };
type Workflow = {
  id: string;
  name: string;
  residence: string;
...
function WorkflowsPage() {
  const [workflows, setWorkflows] = useState<Workflow[]>(() => JSON.parse(JSON.stringify(WORKFLOWS)));
  ...
  const updateStep = (wfId: string, stepIdx: number, patch: Partial<Step>) => {
    setWorkflows((prev) =>
      prev.map((w) =>
        w.id !== wfId
          ? w
          : {
              ...w,
              steps: w.steps.map((s, i) => (i === stepIdx ? { ...s, ...patch, updated: nowStamp() } : s)),
            },
      ),
    );
  };
        ...
        w.id !== wfId
          ...
          : {
              ...
              steps: w.steps.map((s, i) =>
                i === stepIdx ? { ...s, attachments: [...(s.attachments ?? []), { name: fileName }] } : s,
              ),
            },
      ),
    );
  ...
  const notifyAssignee = (assignee: string, context: string) => {
    toast.success(`Notification sent to ${assignee}`, { description: context });
  };

  const createWorkflow = (data: { name: string; residence: string; budget: string }) => {
    const newWf: Workflow = {
      id: `w${Date.now()}`,
      name: data.name,
      residence: data.residence,
      budget: data.budget,
      steps: WORKFLOW_STEPS.map((name, i) => ({
        name,
    ...
    setCreateOpen(false);
    toast.success("Workflow created", { description: newWf.name });
  };
